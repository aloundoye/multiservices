...
const initialDebt = { customerName: "", phone: "", accountId: "", amount: 0, issuedAt: today(), dueDate: "", note: "" };
...
export function DebtsPage({ onChanged, notify }: { onChanged: () => void; notify: (message: string) => void }) {
  ...
  const [payment, setPayment] = useState({ amount: 0, accountId: "", paidAt: today(), note: "" });
  ...
  async function pay(event: FormEvent) {
    ...
    try {
      ...
      setPaymentOpen(false); setPayment({ amount: 0, accountId: "", paidAt: today(), note: "" }); await load(selected.id); onChanged(); notify("Remboursement enregistré.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setLoading(false); }
  }
