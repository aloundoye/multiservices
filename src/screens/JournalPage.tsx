  ...
  return (
    <div className="page">
      ...
      <section className="panel table-panel">
        ...
        {visible.length === 0 ? <div className="empty-state"><BookOpenText /><h3>Aucune écriture</h3><p>Ajoutez une recette, un achat ou une dépense.</p></div> : <div className="table-scroll"><table><thead><tr><th>Date</th><th>Type</th><th>Compte</th><th>Référence / note</th><th>Montant</th><th></th></tr></thead><tbody>{visible.map((item) => <tr className={item.reversed || item.reversesId ? "muted-row" : ""} key={item.id}><td>{formatDate(item.occurredAt)}</td><td><span className={`entry-type ${item.signedAmount >= 0 ? "income" : "outcome"}`}>{item.productOperation ? (item.productOperation.kind === "sale" ? "Vente de produits" : "Réapprovisionnement") + (item.reversesId ? " — Annulation" : "") : label(item.entryType)}</span>{item.reversed && <small>Corrigée</small>}</td><td>{accountDisplayLabel(item.accountSnapshot)}</td><td><strong>{item.reference || "—"}</strong><small>{item.note || "Aucune note"}</small></td><td><strong className={item.signedAmount >= 0 ? "positive" : "negative"}>{signed(item.signedAmount)}</strong></td><td>{!item.reversed && !item.reversesId && <button className="icon-button" title="Créer une contre-écriture" onClick={() => { setError(""); setReverseTarget(item); }}><RotateCcw size={17} /></button>}</td></tr>)}</tbody></table></div>}
      </section>
      ...
      <Modal title="Nouvelle écriture" subtitle="Le capital attendu sera ajusté automatiquement." open={open} onClose={() => setOpen(false)}>
        <form className="modal-form" onSubmit={submit}>
          ...
          {["sale", "purchase"].includes(form.entryType) && <p className="stock-hint">Pour les produits suivis en stock, utilisez « Produits et ventes » : leurs ventes et achats sont déjà ajoutés automatiquement au journal.</p>}
          <div className="form-grid"><Field label="Montant"><MoneyInput autoFocus value={form.amount} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) })} required /></Field><Field label="Compte utilisé"><AccountSelect value={form.accountId} onChange={(accountId) => setForm({ ...form, accountId })} /></Field></div>
      ...
      <Modal title="Corriger cette écriture" subtitle={reverseTarget?.productOperation ? "Cette annulation corrige le stock et le budget. La vente doit être intégralement remboursée et les articles repris ; pour un achat, les articles doivent être rendus et le montant récupéré." : "Une contre-écriture sera ajoutée; l’original restera visible."} open={Boolean(reverseTarget)} onClose={() => setReverseTarget(undefined)}>
