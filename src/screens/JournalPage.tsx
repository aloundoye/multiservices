  ...
  return (
    <div className="page">
      ...
      <section className="panel table-panel">
        ...
        {visible.length === 0 ? <div className="empty-state"><BookOpenText /><h3>Aucune écriture</h3><p>Ajoutez une recette, un achat ou une dépense.</p></div> : <div className="table-scroll"><table><thead><tr><th>Date</th><th>Type</th><th>Compte</th><th>Référence / note</th><th>Montant</th><th></th></tr></thead><tbody>{visible.map((item) => <tr className={item.reversed || item.reversesId ? "muted-row" : ""} key={item.id}><td>{formatDate(item.occurredAt)}</td><td><span className={`entry-type ${item.signedAmount >= 0 ? "income" : "outcome"}`}>{label(item.entryType)}</span>{item.reversed && <small>Corrigée</small>}</td><td>{accountDisplayLabel(item.accountSnapshot)}</td><td><strong>{item.reference || "—"}</strong><small>{item.note || "Aucune note"}</small></td><td><strong className={item.signedAmount >= 0 ? "positive" : "negative"}>{signed(item.signedAmount)}</strong></td><td>{!item.reversed && !item.reversesId && <button className="icon-button" title="Créer une contre-écriture" onClick={() => { setError(""); setReverseTarget(item); }}><RotateCcw size={17} /></button>}</td></tr>)}</tbody></table></div>}
      </section>
